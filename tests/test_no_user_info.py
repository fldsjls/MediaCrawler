...
3. 跨平台源码断言 —— store/ 不再把禁用字段作为存储 dict 的 key。
"""
import re
import pathlib
...
def test_store_no_forbidden_dict_keys():
    ...
    matches = _store_source_matches('"(' + "|".join(FORBIDDEN_KEYS) + r')"\s*:')
...
def test_store_no_creator_orm_imports():
    ...
    matches = _store_source_matches(r"from database\.models import.*(XhsCreator|DyCreator|WeiboCreator|TiebaCreator|ZhihuCreator|BilibiliUpInfo|BilibiliContactInfo)")

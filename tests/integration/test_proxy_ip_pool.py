...
from mediacrawler.infrastructure.proxy.proxy_ip_pool import create_ip_pool, ProxyIpPool
from mediacrawler.infrastructure.proxy.types import IpInfoModel
...
class TestIpPool(IsolatedAsyncioTestCase):
    ...
    async def test_create_ip_pool_unknown_provider_raises_value_error(self):
        ...
        from mediacrawler.infrastructure.proxy.proxy_ip_pool import IpProxyProvider
        ...
        for bad_name in invalid_cases:
            with self.subTest(provider=bad_name):
                with patch("mediacrawler.infrastructure.proxy.proxy_ip_pool.config") as mock_config:
                    mock_config.IP_PROXY_PROVIDER_NAME = bad_name
                    with self.assertRaises(ValueError) as ctx:
                        from mediacrawler.infrastructure.proxy.proxy_ip_pool import create_ip_pool

...
visit('index.md');
const migrations = JSON.parse(await readFile(path.join(root, '.vitepress/redirects.json'), 'utf8'));
for (const [old, target] of Object.entries(migrations)) {
  if (markdown.has(old) || !markdown.has(target)) errors.push(`Invalid redirect: ${old} -> ${target}`);
}
for (const name of markdown.keys()) {
  if (!reached.has(name)) errors.push(`Page is not reachable from homepage: ${name}`);
}

...
def install(check_only=False):
    ...
    local = Path(os.environ.get("MC_LOCAL_DIR", PROJECT / ".local")).expanduser().resolve()
    tools = local / "tools"
    cache = local / "cache" / "setup"
        ...
        for filename, hashes in item["files"].items():
            ...
            if not destination.is_file():
                ...
                raise RuntimeError(f"Unrecognized {destination}; existing files are never overwritten. "
                                   "Back up the file and review scripts/setup/workbench-tools.json before replacement.")
        if not missing:

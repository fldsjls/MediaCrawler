...
try {
    ...
    foreach ($directory in @($projectRoot, (Join-Path $projectRoot 'src\webui'), (Join-Path $projectRoot 'src\browser-worker'))) {
    ...
    if (-not $SkipChecks) { & (Join-Path $projectRoot 'scripts/validation/check-workbench.ps1') -SkipBuild }
    Write-Host 'Installed. Start with: powershell -NoProfile -ExecutionPolicy Bypass -File scripts\runtime\start-workbench.ps1'
} finally {
    Pop-Location

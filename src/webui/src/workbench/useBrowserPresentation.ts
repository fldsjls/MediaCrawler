...
// ICE (5s), server startup/SDP (25s plus cleanup), then first decoded frame.
// Keep the overall deadline outside those phase budgets, so a valid cold start
// does not get aborted before its answer or detailed failure reaches the UI.
const RTC_REQUEST_TIMEOUT_MS = 28000
const RTC_CONNECT_TIMEOUT_MS = 40000
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        timer = setTimeout(() => { controller.abort(); fail('实时画面连接超时') }, RTC_CONNECT_TIMEOUT_MS)
        ...
        const requestTimer = setTimeout(() => controller.abort(), RTC_REQUEST_TIMEOUT_MS)

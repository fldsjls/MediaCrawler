...
interface Props { sourceType?:string; session?: Session; visible: boolean; target: string; onOpen: () => void; onSession: (session: Session) => void; onError: (message: string) => void; recentTasks?: Task[]; onOpenTask?: (task: Task) => void; navigationRequest?: NavigationRequest }
type ControlMessage = { action: string; [key: string]: unknown }
type WheelInput = { x: number; y: number; dx: number; dy: number }
export function BrowserPanel({ sourceType='website', session, visible, target, onOpen, onSession, onError, recentTasks = [], onOpenTask, navigationRequest }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => job.finish('打开网站超时，请检查当前页面后重试'), ((session?.navigation_timeout||30)+5)*1000)
  ...
  if (!session) return <div className="wb-empty"><span className="wb-eyebrow">当前目标</span><p>{target || '先选择平台并填写目标。'}</p>{sourceType==='website'?<><ol className="wb-preview-steps"><li>打开网站并登录</li><li>播放以发现资源</li><li>开始采集或手选下载</li></ol><button onClick={onOpen}>打开网站</button><small>打开网站和播放只发现资源，不会自动下载。</small><small>支持 Chromium / Edge · 预览不会录制保存</small></>:<p>{sourceType==='history'?'使用历史运行数据，在左侧配置导出步骤后开始任务。':'直接文件已作为方案输入，在左侧配置下载步骤后开始任务。'}</p>}{recentTasks.length > 0 && onOpenTask && <div className="wb-recent"><strong>近期任务与结果</strong>{recentTasks.map(task => <button key={task.id} onClick={() => onOpenTask(task)} title={task.config.target}><span>{task.config.target}</span><small>{states[task.state]}</small></button>)}</div>}</div>

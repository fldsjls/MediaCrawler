...
export function TaskCenter({ tasks, platforms, sessions, busy, onOpen, onNew, onRefresh, onControl, onError }: Props) {
  ...
  const visible = tasks.filter(task => (filter === 'all' || group.states.includes(task.state))
    ...
    && `${task.name || ""} ${task.config.target} ${name(task.config.platform)} ${task.id}`.toLowerCase().includes(query.toLowerCase()))
  ...
  return <section className="wb-page wb-task-center" aria-labelledby="task-center-title">
    <div className="wb-page-heading">
      ...
      <div className="wb-heading-actions"><button disabled={busy} onClick={onRefresh}><RefreshCw size={16} />刷新</button><button className="wb-primary" onClick={onNew}><Plus size={16} />新建任务</button></div>
    </div>
    ...
    <div className="wb-table-scroll wb-task-table-wrap">
      <table className="wb-table wb-task-table" aria-label="任务运行">
        <thead><tr><th>任务名称</th><th>平台</th><th>任务状态</th><th>当前步骤</th><th>文件完成 / 失败</th><th>创建时间</th><th>操作</th></tr></thead>
        <tbody>{visible.map(task => <tr key={task.id} data-selected={selected?.id === task.id} onClick={event => showDetails(task, event.currentTarget.querySelector<HTMLButtonElement>('.wb-title-button')!)}>
          <td><button className="wb-title-button" title={task.config.target} aria-haspopup="dialog" onClick={event => { event.stopPropagation(); showDetails(task, event.currentTarget) }}>{task.name || task.config.target || '下载任务'}</button></td>
          <td>{name(task.config.platform)}</td>
          <td><div className="wb-task-status"><StatusBadge state={task.state} /></div></td>
          <td>{task.current_step || '—'}<small>步骤 {task.steps_completed ?? 0} / {task.steps?.length ?? 1}</small></td><td>{task.counts.files_succeeded} / {task.counts.files_failed}</td>
      ...
      {!visible.length && <div className="wb-table-empty"><h3>{tasks.length ? '没有匹配的任务' : '还没有采集任务'}</h3><p>{tasks.length ? '尝试其他关键词、平台或任务状态。' : '创建采集后，可以在这里查看进度与结果。'}</p>{!tasks.length && <button onClick={onNew}>新建任务</button>}</div>}
    </div>

...
export function TaskDetailsDrawer({ task, platformName, busy, onClose, onOpen, onControl }: Props) {
  ...
  const [view, setView] = useState(task.steps?.some(s=>['media','files','export'].includes(s.kind)) || task.counts.files_succeeded ? 'files' : 'data')
  ...
  return <dialog ref={dialog} className="wb-task-drawer" aria-labelledby="task-detail-title" onCancel={event => { event.preventDefault(); onClose() }} onKeyDown={event => {
    ...
    <div className="wb-tabs wb-task-detail-tabs" role="tablist" aria-label="运行详情">{[['steps','步骤'],['files','文件'],['data','数据'],['logs','日志']].map(([id,title])=><button key={id} role="tab" aria-selected={view===id} onClick={()=>setView(id)}>{title}</button>)}</div>
    {(view==='files'||view==='data')&&<Artifacts taskId={task.id} view={view}/>}
    {view==='steps'&&<div className="wb-run-content wb-run-steps"><p>{task.name || task.config.target}</p>{task.retry_of&&<p>重试来源：{task.retry_of}</p>}{task.steps?.map(s=><section className="wb-run-step" key={s.id}><strong>{s.name}</strong> <StatusBadge state={s.state}/>{s.error&&<p className="wb-detail-error">{s.error}</p>}<details><summary>本次实际设置</summary><pre>{JSON.stringify(s.settings,null,2)}</pre></details></section>)}</div>}
    <div role="tabpanel" className="wb-task-drawer-body" hidden={view !== 'logs'}>

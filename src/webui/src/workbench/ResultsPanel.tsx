...
export function ResultsPanel({ results, taskId, onExport, resources = noResources, sessionId, busy = false, onDownload, resourceStatus }: Props) {
  ...
  return <div className="wb-results">
    ...
    {view === 'videos' ? filtered.length ? <ResourceTable key={`${taskId || ''}:${sessionId || ''}`} resources={filtered} selected={selection} busy={busy} canDownload={Boolean(onDownload)} onToggle={toggle} onDownload={requestDownload} /> : <div className="wb-table-empty"><Video size={32} strokeWidth={1.5} /><h3>{videos.length ? '没有匹配的视频' : '尚未发现视频'}</h3><p>{videos.length ? '尝试其他关键词或状态。' : '打开网站并播放视频，或开始采集后，发现的视频会列在这里。'}</p>{resourceStatus && <small>{resourceStatus}</small>}</div> : <div className="wb-result-scroll">
      ...
      {(view === 'files' || view === 'failures') && <>{!files.length && (view !== 'failures' || !failed.length) && <div className="wb-table-empty"><FileText size={30} /><h3>{view === 'files' ? '暂无下载文件' : '没有失败项'}</h3></div>}{files.map(f => <div className="wb-record wb-file-row" key={f.id}><FileText size={22} /><strong>{f.title}</strong><StatusBadge state={f.state} resource /><small>{bytes(f.size)}</small>{f.path && taskId && <a href={fileUrl(taskId, f.path)}>下载文件</a>}{f.error && <p className="wb-detail-error">{f.error}</p>}</div>)}</>}
      {view === 'failures' && failed.map(r => <div className="wb-record" key={r.id}><StatusBadge state="failed" />{r.title}<small>{r.target}</small></div>)}
    </div>}
    {taskId && results.exports.length > 0 && <div className="wb-export-shelf"><strong>导出文件</strong><div>{results.exports.map(path => <a key={path} href={fileUrl(taskId, path)}><FileText size={18} />{path}</a>)}</div></div>}

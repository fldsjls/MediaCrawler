...
export function Workbench({ onConnectionChange }: { onConnectionChange?: (state: 'connecting' | 'connected' | 'disconnected') => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (version === sessionVersion.current) setSessions(nextSessions)
      setOffline(false); onConnectionChange?.('connected')
    } catch (error) {
      if (request === refreshRequest.current) throw error
    }
  }, [onConnectionChange])
  useEffect(() => {
    let stopped = false
    ...
    void poll()
    return () => { stopped = true; clearTimeout(timer); refreshRequest.current++ }
  ...
  const openWebsite = (external = false) => void perform(async () => {
    if (!platform) return
    const existing = sessions.find(s => s.platform === config.platform && (!s.task_id || s.finished))
    const url = config.mode !== 'search' && /^https?:\/\//.test(config.target.trim()) ? config.target.split(',')[0].trim() : platform.url
    if (!url) throw new Error('请先填写网页地址')
    if (existing && !external) {
      setPreviewId(existing.id)
      if (existing.pages.find(page => page.selected)?.url !== url) await new Promise<void>((resolve, reject) => {
        const request: NavigationRequest = { sessionId: existing.id, url, complete: error => {
          setNavigationRequest(current => current === request ? undefined : current)

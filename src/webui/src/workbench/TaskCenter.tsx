...
import { ended, Platform, Task } from './api'
import { StatusBadge } from './components/StatusBadge'

interface Props {
  tasks: Task[]; platforms: Platform[]; selected?: Task; busy: boolean; logs: ReactNode
  onSelect: (task: Task) => void; onOpen: (task: Task) => void; onNew: () => void
...
export function TaskCenter({ tasks, platforms, selected, busy, logs, onSelect, onOpen, onNew, onRefresh, onControl }: Props) {
  ...
  return <section className="wb-page wb-task-center" aria-labelledby="task-center-title">
    <div className="wb-page-heading">
      <div><h1 id="task-center-title">任务中心</h1><p>管理采集任务与运行状态</p></div>
    ...
    <div className="wb-table-scroll wb-task-table-wrap">
      <table className="wb-table wb-task-table" aria-label="采集任务">
        ...
        <tbody>{visible.map(task => <tr key={task.id} data-selected={selected?.id === task.id} onClick={() => onSelect(task)}>
          <td><button className="wb-title-button" title={task.config.target} onClick={() => onSelect(task)}>{task.config.target || '媒体下载任务'}</button></td>
          ...
          <td>{task.counts.content} / {task.config.max_items}</td><td>{task.counts.files_succeeded} / {task.config.max_downloads}</td>
          <td className="wb-date">{date(task.created)}</td>
          <td><div className="wb-row-actions" onClick={e => e.stopPropagation()}>
            <button onClick={() => onOpen(task)} aria-label={`查看任务 ${task.config.target}`}><Eye size={15} />查看</button>
    ...
    {selected && <div className="wb-task-details">
      <section aria-label="任务基本信息"><h2>基本信息</h2><dl className="wb-detail-grid">
        <dt>任务目标</dt><dd>{selected.config.target}</dd><dt>任务状态</dt><dd><StatusBadge state={selected.state} /></dd>
        <dt>平台</dt><dd>{name(selected.config.platform)}</dd><dt>导出格式</dt><dd>{selected.config.output.toUpperCase()}</dd>
        <dt>内容 / 上限</dt><dd>{selected.counts.content} / {selected.config.max_items}</dd><dt>评论</dt><dd>{selected.counts.comments}</dd>
        <dt>创建时间</dt><dd>{date(selected.created)}</dd>
      </dl>{selected.error && <p className="wb-detail-error">{selected.error}</p>}</section>
      <section className="wb-task-detail-logs" aria-label="所选任务日志"><h2>运行日志</h2>{logs}</section>
    </div>}

...
from mediacrawler.workbench.browser.capture import classify, manifest_status
...
class MediaProxy:
    ...
    async def __aenter__(self):
        self.client = httpx.AsyncClient(timeout=httpx.Timeout(getattr(self.run, 'download_settings', {}).get('timeout', 60)), trust_env=False)
                ...
                if format in ('hls', 'dash'):
                    ...
                    if method != 'HEAD':
                        async for block in response.aiter_bytes():
                            ...
                            callback = getattr(self.run, 'download_progress', None)
                            if callback: callback(self.received, 0)
                            await writer.drain()
...
async def direct(run, resource, path):
    async with httpx.AsyncClient(timeout=getattr(run, 'download_settings', {}).get('timeout', 60), trust_env=False) as client:
        async with scoped_request(client, run, resource, web_url(resource['url'])) as (response, _url):
            ...
            with path.open('wb') as output:
                async for block in response.aiter_bytes():
                    ...
                    output.write(block)
                    received += len(block)
                    callback = getattr(run, 'download_progress', None)
                    if callback: callback(received, int(response.headers.get('content-length', 0)))
            if response.headers.get('content-length') and received != int(response.headers['content-length']):
                raise RuntimeError('下载长度不完整')
...
async def download(run, item, directory, toolspath):
    ...
    if settings.get('ffmpeg_path'): executable = Path(settings['ffmpeg_path'])
    ...
    from .engines import select_engine, n_m3u8dl
    engine = select_engine(settings, {**item, 'format': format}) if settings else ('ffmpeg' if mux else 'http')
    run.actual_engine = engine
    if settings.get('quality', 'best') != 'best' and (item.get('kind') == 'video' or mux):
        if (not mux or composite) and str(item.get('height', '')) != settings['quality']:
            raise ValueError('固定文件或分离流没有所选清晰度，请使用默认清晰度或选择对应资源')
        item['height'] = int(settings['quality'])
    timeout = str(int(settings.get('timeout', 60)) * 1_000_000)
    audio_map = '0:a?' if settings.get('audio') == 'all' else '0:a:0?'
    suffix = Path(urlsplit(item.get('url', '')).path).suffix.lower()
    extension = '.mp4' if mux or engine == 'ffmpeg' else suffix if re.fullmatch(r'\.[a-zA-Z0-9]{1,10}', suffix) else '.bin'
    destination = directory / f'{identity}-{name}{extension}'
    if destination.exists():
        if settings.get('collision') == 'error': raise ValueError('同名文件已存在，未覆盖')
        import uuid
        destination = destination.with_stem(destination.stem + '-' + uuid.uuid4().hex[:8])
    partial = destination.with_suffix('.part' + extension)
    try:
        if engine == 'n_m3u8dl':
            await n_m3u8dl(run, item, partial, settings)
        elif engine == 'http':
                ...
                if segments:
                    ...
                    try:
                        for index, segment in enumerate(segments):
                            ...
                            parts.append(part)
                            await ffmpeg(run, executable, ['-protocol_whitelist', 'http,tcp,crypto', '-rw_timeout', timeout,
                                '-i', proxy.register(resource['url'], resource), '-map', '0:v:0?', '-map', audio_map, '-c', 'copy', '-f', 'mpegts', str(part)])
                    ...
                    for stream in streams:
                        ...
                        if 'headers' in stream:
                            resource['_header_url'] = resource['url']
                        args += ['-protocol_whitelist', 'http,tcp,crypto', '-rw_timeout', timeout, '-i', proxy.register(resource['url'], resource)]
                    if len(streams) > 1:
        ...
        if mux:
            with partial.open('rb') as stream:
                if b'ftyp' not in stream.read(64): raise RuntimeError('合并产物不是有效的 MP4 容器')

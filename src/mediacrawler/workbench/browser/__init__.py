...
from mediacrawler.workbench.workflows.legacy_models import PLATFORMS, SessionConfig, web_url
...
from mediacrawler.workbench.browser.preview.native import NativeRTCStreams
from mediacrawler.workbench.browser.preview.internal import public_pages, INTERNAL_PREVIEW_PATH
from mediacrawler.workbench.settings import BrowserSettings
...
class BrowserSession:
    ...
    async def start(self):
        ...
        profile_name = f'{self.config.platform}-{self.config.channel}' if self.preview_settings.get('reuse_login', True) else f'isolated-{self.id}'
        profile = self.root / 'profiles' / profile_name
        ...
        if self.config.url:
            try:
                await self.page.goto(web_url(self.config.url), wait_until='domcontentloaded', timeout=self.preview_settings.get('navigation_timeout', 30)*1000)
    ...
    def snapshot(self):
        return dict(id=self.id, platform=self.config.platform, token=self.token, manual=self.manual,
                    ...
                    width=1280, height=720, navigation_timeout=self.preview_settings.get('navigation_timeout',30), page_epoch=self.frame_hub.epoch, presentation=self.presentation_snapshot(),
    ...
    async def input(self, message, owner):
        async with self.control_lock:
            ...
            if action == 'navigate':
                await page.goto(web_url(message['url']), wait_until='domcontentloaded', timeout=self.preview_settings.get('navigation_timeout', 30)*1000)
            elif action == 'reload':
                await page.reload(wait_until='domcontentloaded', timeout=self.preview_settings.get('navigation_timeout', 30)*1000)
            elif action == 'click':
...
class BrowserSessions:
    ...
    async def create(self, config, *, platform_snapshot=None, settings_snapshot=None):
        platform = platform_snapshot or (self.platform_lookup(config.platform) if self.platform_lookup else next(
        ...
        async with self.lock:
            ...
            settings = settings_snapshot or (self.settings_lookup() if self.settings_lookup else BrowserSettings().model_dump())

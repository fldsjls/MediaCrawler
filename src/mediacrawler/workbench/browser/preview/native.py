...
from mediacrawler.workbench.browser.preview.internal import INTERNAL_PREVIEW_PATH, is_internal_preview_url
from mediacrawler.workbench.browser.preview.viewport import fit_native_viewport

# Covers cold helper startup, capture, viewport alignment and native SDP work.
# The web client's request budget must also leave time for cancellation cleanup.
NATIVE_OFFER_TIMEOUT_SECONDS = 25
...
class NativeRTCStreams:
    ...
    async def _capture(self, page):
        ...
        try:
            ...
            # The static button is already loaded. Avoid locator resolution and
            # stability polling, which can depend on background rAF scheduling.
            self.phase = '请求标签页捕获'
            await helper.bring_to_front()
            point = await helper.evaluate('''() => {
                const button = document.getElementById('capture');
                const rect = button?.getBoundingClientRect();
                if (!rect || rect.width <= 0 || rect.height <= 0) throw new Error('内部捕获按钮不可用');
                return {x:rect.left + rect.width / 2,y:rect.top + rect.height / 2};
            }''')
            await helper.mouse.click(point['x'], point['y'])
            for _ in range(160):
    ...
    async def offer(self, sdp, description_type):
        try:
            async with asyncio.timeout(NATIVE_OFFER_TIMEOUT_SECONDS):
                return await self._offer(sdp, description_type)
        except TimeoutError:
            self.error = f'原生实时预览连接超时（{self.phase}），请重试或使用低刷新预览'
        ...
        try:
            async with self.session.control_lock:
                async with self.lock:
                    ...
                    helper = self.helper
                    # Reserve the shared stream while still holding the same
                    # lock used by close_peer; ICE negotiation follows unlocked.
                    await helper.evaluate('id => NativePreview.reserve(id)', peer_id)

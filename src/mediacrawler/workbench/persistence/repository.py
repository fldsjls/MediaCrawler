...
from mediacrawler.workbench.workflows.legacy_models import TERMINAL
...
class Repository:
    ...
    def __init__(self, root: Path):
        ...
        if version < 4 and self.db.execute("SELECT 1 FROM sqlite_master WHERE name='tasks'").fetchone():
            target_version = 4
        ...
        self.db.executescript('''
          ...
          CREATE TABLE IF NOT EXISTS workflow_plans(id TEXT PRIMARY KEY, payload TEXT NOT NULL, updated REAL NOT NULL);
          CREATE TABLE IF NOT EXISTS workflow_steps(task_id TEXT, id TEXT, payload TEXT NOT NULL, PRIMARY KEY(task_id,id));
        ...
        self.db.commit()
        self.step_context = {}
        for row in self.db.execute('SELECT task_id,id,payload FROM workflow_steps').fetchall():
            payload = json.loads(row['payload'])
            if payload['state'] not in TERMINAL and payload['state'] not in ('skipped', 'blocked'):
                payload['state'] = 'interrupted'
                self.step(row['task_id'], payload)
    ...
    def record(self, task_id, item):
        item = dict(item)
        item.setdefault('step_id', self.step_context.get(task_id, ''))
        if item.get('step_id') and item.get('id') and not item.get('original_id'):
            item['original_id'] = item['id']
            item['id'] = item['step_id'] + ':' + item['id']
    ...
    def resource(self, task_id, item):
        item = {**item, 'step_id': item.get('step_id') or self.step_context.get(task_id, '')}
    ...
    def step(self, task_id, payload):
        with self.db:
            self.db.execute('INSERT OR REPLACE INTO workflow_steps VALUES(?,?,?)', (task_id, payload['id'], json.dumps(payload, ensure_ascii=False)))

    def steps(self, task_id):
        return [json.loads(r[0]) for r in self.db.execute('SELECT payload FROM workflow_steps WHERE task_id=? ORDER BY rowid', (task_id,))]

    def save_plan(self, payload, plan_id=None):
        plan_id = plan_id or uuid.uuid4().hex
        with self.db:
            self.db.execute('INSERT OR REPLACE INTO workflow_plans VALUES(?,?,?)', (plan_id, json.dumps(payload, ensure_ascii=False), now()))
        return {'id': plan_id, **payload}

    def plans(self):
        return [{'id': r['id'], **json.loads(r['payload'])} for r in self.db.execute('SELECT * FROM workflow_plans ORDER BY updated DESC')]

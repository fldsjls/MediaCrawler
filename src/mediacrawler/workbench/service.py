...
from .media.identity import resource_identity, logical_resource_identity  # compatibility import for existing integrations
...
class Workbench:
    ...
    def enqueue_resource(self, run, resource, manual=False):
        ...
        if not existing and not manual and run.retry and media_kind == 'video' and logical_id:
            ...
            if not replacements and any(item['state'] == 'running' for item in automatic):
                ...
                return
        if not existing and len(files) - len(replacements) >= run.config.max_downloads:
            return
    ...
    async def prepare_session(self, run):
        ...
        for old in list(self.sessions.items.values()):
            ...
            if disconnected or old is not session and old.task_id and not reserved:
                await old.close()
    ...
    async def collect(self, run):
        ...
        command = ['node', '--import', 'tsx', 'src/workbench/worker.ts'] if node else [sys.executable, '-m',
            'mediacrawler.workbench.template_worker' if not definition['builtin'] else 'mediacrawler.workbench.platform_worker']

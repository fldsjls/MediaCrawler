...
(() => {
  ...
  const closePeer = id => {
    ...
    if (!peers.size && !reservations.size) stopCapture();
  };
  const reset = () => { generation++; reservations.clear(); for (const id of [...peers.keys()]) closePeer(id); stopCapture(); };
  const matches = value => {
  ...
  window.NativePreview = {
    ...
    async answer(value) {
      if (!reservations.has(value.id)) throw new Error('实时预览连接已取消');
      if (!matches(expected) || needsIdentity) throw new Error('标签页身份尚未验证');
    ...
    state() {
      return { ids: [...peers.keys()], reserved: [...reservations], capture: Boolean(stream), needsIdentity, error: lastError,

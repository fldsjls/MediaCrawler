...
const DEFAULT_INPUT = 'storage/meishiwang/export/meishiwang-m3u8-results.json';
const DEFAULT_OUTPUT_DIR = 'storage/meishiwang/downloads';
const DEFAULT_LOG_DIR = 'logs/downloads';
const DEFAULT_N_M3U8DL = process.platform === 'win32' ? 'tools/n-m3u8dl-re/N_m3u8DL-RE.exe' : 'N_m3u8DL-RE';
const DEFAULT_FFMPEG = process.platform === 'win32' ? 'tools/ffmpeg/ffmpeg.exe' : 'ffmpeg';
...
Options:
  ...
  --input <path>           Crawlee m3u8 result JSON. Default: storage/meishiwang/export/meishiwang-m3u8-results.json.
  --downloader <path>      Downloader executable path.
  --ffmpeg-path <path>     FFmpeg path passed to N_m3u8DL-RE. Default: tools/ffmpeg/ffmpeg.exe.
  --output-dir <path>      Download output directory. Default: storage/meishiwang/downloads.
  --temp-dir <path>        Optional downloader cache/temp directory. Empty by default.
  --log-dir <path>         Download log directory. Default: logs/downloads.
...
Examples:
  ...
  npm run download:m3u8 -- --limit 3 --use-temp --temp-dir storage/temp
  npm run download:m3u8 -- --engine ffmpeg --input storage/meishiwang/export/meishiwang-m3u8-results.json
`;
}

...
Options:
  ...
  --output <path>     JSON summary output. Default: logs/m3u8-results.json.
  ...
  --video-dir <path>  Video output directory. Default: logs/videos/m3u8.
  --no-red-boxes      Disable red boxes if enabled by a wrapper script.
  --screenshot-dir    Screenshot output directory. Default: logs/screenshots/m3u8.
...
const outputPath = resolve(options.output ?? genericM3u8Settings.output ?? 'logs/m3u8-results.json');

...
> 文档类型：入门操作
> 环境：当前 Windows 本机主项目；代码依据：`scripts/install-workbench.ps1`、`scripts/start-workbench.ps1`。
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\check-workbench.ps1 -SkipTests
# 单独补齐下载工具；已有且哈希匹配的版本保留
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\install-workbench-tools.ps1
```
...
安装工具源和 SHA256 固定在 `scripts/workbench-tools.json`。FFmpeg 当前迁入版本与固定安装版本均在许可和来源清单中记录。缺失工具从固定官方发行地址下载，先验压缩包哈希再验可执行文件哈希；已有未知哈希文件报错并保留，不自动覆盖。下载缓存位于 `data/workbench/setup-cache`，版本升级需明确更新清单。


下一步：[完成一次采集](/guides/collection.md)。需要旧平台 CLI 时阅读[CLI 参考](/guides/cli.md)。

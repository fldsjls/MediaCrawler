...
1. **Configure Excel export** in `src/mediacrawler/config/base_config.py`:
...
# Douyin example
uv run mediacrawler --platform dy --lt qrcode --type search

# Bilibili example
uv run mediacrawler --platform bili --lt qrcode --type search
```
...
uv run mediacrawler --platform xhs --lt qrcode --type detail --save_data_option excel

# Crawl creator profile and export to Excel
uv run mediacrawler --platform xhs --lt qrcode --type creator --save_data_option excel
```
...
- Check [常见问题](/operations/cli-faq.md)

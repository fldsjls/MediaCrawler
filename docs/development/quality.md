...
`tests/test_workflow_plans.py` 覆盖前置输出、排序与禁用依赖、覆盖恢复、提交幂等、默认快照、空历史输入、取消与中断、关联尝试、产物索引和分页预览。真实本地素材分别验证 HTTP、FFmpeg、N_m3u8DL-RE，并通过独立 Chromium 验证网页发现接一个或两个下载消费者再导出。

浏览器验收需检查卡片长文字的实际溢出提示、弹窗保存取消、未保存确认、Esc 与焦点返回、合法排序、空列表无残留详情，以及文件预览不修改方案。桌面和窄屏使用同一方案；本地 HTTP 素材通过不代表真实站点登录、课程权限、平台更新或完整下载覆盖已经验收。

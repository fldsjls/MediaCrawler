...
export function ConfigurationPanel({ config, setConfig, platforms, platform, task, busy, onPlatform, onOpen, onStart, onControl, onError, category, types, onCategory }: Props) {
  ...
  return <aside className="wb-config" aria-label="采集配置">
    <div className="wb-config-heading"><div><span className="wb-eyebrow">采集配置</span><h1>目标与内容</h1></div></div>
    <div className="wb-config-scroll">
      <fieldset><legend>来源与目标</legend>
        ...
        <label>{config.mode === 'search' ? '关键词' : '目标地址或 ID'}<textarea aria-label="目标地址或关键词" rows={3} value={config.target} onChange={e => change('target', e.target.value)} placeholder={platform?.url || '填写目标地址'} /></label>
      </fieldset>

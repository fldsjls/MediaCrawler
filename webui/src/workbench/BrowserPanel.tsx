...
interface Props { session?: Session; visible: boolean; target: string; onOpen: () => void; onSession: (session: Session) => void; onError: (message: string) => void; recentTasks?: Task[]; onOpenTask?: (task: Task) => void }
type ControlMessage = { action: string; [key: string]: unknown }
type WheelInput = { x: number; y: number; dx: number; dy: number }
export function BrowserPanel({ session, visible, target, onOpen, onSession, onError, recentTasks = [], onOpenTask }: Props) {
  ...
  const sendRaw = (message: ControlMessage) => {
    ...
    awaiting.current.push(message.action)
  ...
  useEffect(() => {
    ...
    ws.onclose = () => { manual.current = false; clearWheel(); awaiting.current = []; wheelInFlight.current = 0; setControlConnected(false) }
    ws.onmessage = event => {
      const message = JSON.parse(event.data)
      let acknowledged: string | undefined
      if (message.type === 'ack' || message.type === 'error') {
        acknowledged = awaiting.current.shift()
        if (acknowledged === 'wheel') wheelInFlight.current = Math.max(0, wheelInFlight.current - 1)
      }
      if (message.type === 'error') { clearWheel(); if (acknowledged === 'select') display.retry(); callbacks.current.onError(message.message) }
      if (message.session) {
    ...
    return () => { manual.current = false; clearWheel(); awaiting.current = []; wheelInFlight.current = 0; ws.onclose = null; ws.onmessage = null; ws.close(); socket.current = undefined; setControlConnected(false) }
  }, [id, token, retry])

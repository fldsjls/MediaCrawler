...
export interface Task { id: string; state: string; config: Config; created: number; session_id?: string; error: string; counts: Record<string, number> }
export type PreviewPreference = 'auto' | 'realtime' | 'snapshot'
export interface BrowserSettings { auto_switch: boolean; snapshot_fps: 1 | 2 | 5; realtime_fps: 10 | 20 | 30 | 60; default_channel: 'chromium' | 'msedge' }
export interface SettingsSection { id: string; title: string; summary: string; persistence: string }
export interface Session { page_epoch?: number; id: string; token: string; platform: string; manual: boolean; finished: boolean; task_id?: string; external: boolean; navigation_error?: string; presentation?: { preference: PreviewPreference; mode: 'realtime' | 'snapshot'; snapshot_fps: number; realtime_fps: number }; pages: { id: string; url: string; selected: boolean }[] }
...
export interface Resource { id: string; title: string; kind: 'video' | 'image'; format?: string; quality?: string; width?: number; height?: number; size?: number; page_url?: string; parent_id?: string; source?: string; origin?: 'adapter' | 'browser'; state: string; error?: string }
...
export async function api<T>(path: string, data?: unknown, method?: string): Promise<T> {
  const response = await fetch(`/api${path}`, { method: method || (data === undefined ? 'GET' : 'POST'), headers: data === undefined ? undefined : { 'Content-Type': 'application/json' }, body: data === undefined ? undefined : JSON.stringify(data) })

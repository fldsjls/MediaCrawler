...
from .preview.internal import public_pages
...
async def run():
    ...
    core = importlib.import_module(f'media_platform.{module_name}.core')
    ...
    client_module = importlib.import_module(f'media_platform.{module_name}.client')
    install_request_barrier(client_module, runtime)
    from .adapters.media import candidate, parse_media
    from .adapters.media_worker import MediaCoordinator
    media_coordinator = MediaCoordinator(cfg['platform'], cfg, runtime, crawler)
    media_coordinator.install(module_name)

    login_module = importlib.import_module(f'media_platform.{module_name}.login')

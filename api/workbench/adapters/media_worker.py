...
from .media import MediaResult, bilibili_playback, candidate, http_url, parse_media
...
class MediaCoordinator:
    ...
    def install(self, module_name):
        store = importlib.import_module(f'store.{module_name}')
        ...
        if self.platform in ('tieba', 'zhihu'):
            helper = importlib.import_module(f'media_platform.{module_name}.help')
        ...
        async with self.browser_lock:
            async def operate():
                ...
                if await first.is_visible():
                    ...
                    await page.wait_for_timeout(min(self.cfg.get('wait_ms', 1500), 3000))
                rows = await videos.evaluate_all('(els)=>els.map(v=>({url:v.src?.startsWith("blob:")?v.src:(v.currentSrc||v.src||v.querySelector("source")?.src),height:v.videoHeight,width:v.videoWidth}))')
                output = MediaResult(confirmed=True)
                for row in rows:
                    if item := candidate(row):
                        output.videos.append(item)
                return output

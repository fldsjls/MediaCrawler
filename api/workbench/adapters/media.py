...
class ContentHTML(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'video':
            self.in_video += 1
            self.has_video = True
        if tag == 'video' or tag == 'source' and self.in_video:
...
def parse_media(platform, raw, normalized=None):
    ...
    if platform == 'bili':
        ...
        if platform == 'tieba':
            ...
            video = raw.get('video') or {}
            videos = [video]
            for row in (video.get('playlist') or raw.get('playlist') or {}).values():
                videos.append(row)
            ...
            result.known_absent = raw.get('type') in ('answer', 'article') and not video and '<video' not in str(body).lower() and 'data-video' not in str(body).lower()
        for video in videos:
            if not isinstance(video, dict) or not video:
                continue
            result.confirmed = True
            variants.append(candidate(video.get('video_url') or video.get('video_src') or video,
                                      height=video.get('height'), width=video.get('width'), quality=video.get('quality')))
        parser = ContentHTML()
        parser.feed(body if isinstance(body, str) else '')
        variants.extend(parser.videos)
